/**
 * Capture M0.2 verification command output into verification/m0.2/
 * Usage: node scripts/verify-m0.2.mjs
 */
import { spawnSync } from 'node:child_process'
import fs from 'node:fs'
import path from 'node:path'
import { fileURLToPath } from 'node:url'

const root = path.dirname(path.dirname(fileURLToPath(import.meta.url)))
const outDir = path.join(root, 'verification', 'm0.2')
fs.mkdirSync(outDir, { recursive: true })

function run(name, command, args) {
  const logPath = path.join(outDir, `${name}.log`)
  const started = new Date().toISOString()
  const result = spawnSync(command, args, {
    cwd: root,
    encoding: 'utf8',
    shell: true,
    env: process.env,
  })
  const body = [
    `# ${name}`,
    `started: ${started}`,
    `command: ${command} ${args.join(' ')}`,
    `exit_code: ${result.status}`,
    '',
    '--- stdout ---',
    result.stdout ?? '',
    '',
    '--- stderr ---',
    result.stderr ?? '',
    '',
  ].join('\n')
  fs.writeFileSync(logPath, body, 'utf8')
  console.log(`${name}: exit ${result.status} -> ${logPath}`)
  return result.status ?? 1
}

const summary = []
let failed = 0

// Clean install
fs.rmSync(path.join(root, 'node_modules'), { recursive: true, force: true })
failed += run('01-clean-install', 'npm', ['install']) ? 1 : 0
summary.push('01-clean-install')

failed += run('02-lint', 'npm', ['run', 'lint']) ? 1 : 0
summary.push('02-lint')

failed += run('03-unit-integrity', 'npm', ['test']) ? 1 : 0
summary.push('03-unit-integrity')

failed += run('04-build', 'npm', ['run', 'build']) ? 1 : 0
summary.push('04-build')

failed += run('05-playwright', 'npm', ['run', 'test:e2e']) ? 1 : 0
summary.push('05-playwright')

const report = {
  milestone: 'M0.2',
  generatedAt: new Date().toISOString(),
  steps: summary,
  failedSteps: failed,
  claim:
    'Once BranchBack has accepted something as historical record, no ordinary later write may silently alter or erase that history.',
}
fs.writeFileSync(
  path.join(outDir, 'summary.json'),
  JSON.stringify(report, null, 2),
  'utf8',
)
console.log(JSON.stringify(report, null, 2))
process.exit(failed > 0 ? 1 : 0)
