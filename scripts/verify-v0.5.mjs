/**
 * Capture v0.5 verification command output into verification/v0.5/
 */
...
const outDir = path.join(root, 'verification', 'v0.5')
...
try {
  ...
  console.warn('clean install: could not remove node_modules, continuing with npm install', e.message)
}
...
const report = {
  ...
  failedSteps: failed,
  story:
    'Historical semantics + full lifecycle + intelligence views + offline-verified local-first beta',
}
